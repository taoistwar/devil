...
/// 插件权限级别
#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq, Eq, PartialOrd)]
...
impl PluginLoader {
            ...
            if path.is_dir() {
                ...
                if plugin_json.exists() {
                    match self.load_plugin_metadata(&plugin_json) {
                        Ok(metadata) => {
                            self.check_plugin_security(&metadata)?;
    ...
    fn load_plugin_metadata(&self, path: &Path) -> Result<PluginMetadata, PluginLoadError> {
        let content = std::fs::read_to_string(path).map_err(|e| {
            PluginLoadError::InvalidMetadata(format!("Failed to read {:?}: {}", path, e))
        })?;

        let metadata: PluginMetadata = serde_json::from_str(&content).map_err(|e| {
            PluginLoadError::InvalidMetadata(format!("Failed to parse {:?}: {}", path, e))
        })?;
    ...
    fn check_plugin_security(&self, metadata: &PluginMetadata) -> Result<(), PluginLoadError> {
        ...
        if self.security_policy.blocklist.contains(&metadata.name) {
            return Err(PluginLoadError::SecurityCheckFailed(format!(
        ...
        if metadata.permission_level > self.security_policy.max_permission_level {
            return Err(PluginLoadError::SecurityCheckFailed(format!(
                ...
                metadata.name, metadata.permission_level, self.security_policy.max_permission_level
            )));
        }
