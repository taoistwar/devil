...
use crate::tools::tool::{Tool, ToolContext, PermissionResult};
use anyhow::Result;
...
impl PermissionCheckResult {
    ...
    /// 创建带规则匹配结果的拒绝
    pub fn deny_with_rule(rule: PermissionRule, source: RuleSource, reason: impl Into<String>) -> Self {
        Self {
            behavior: PermissionBehavior::Deny {
                reason: reason.into(),
            },
...
impl PermissionPipeline {
    ...
    pub async fn check_permissions<T: Tool>(
        ...
        if !validation_result.is_valid {
            ...
            return Ok(PermissionCheckResult::ask(
                validation_result.error_message.unwrap_or_else(|| "Invalid input".to_string())
            ));
        }
        ...
        let final_result = if matches!(rule_check.behavior, PermissionBehavior::Ask { .. }) {
            rule_check
        } else if matches!(context_check.behavior, PermissionBehavior::Ask { .. } | PermissionBehavior::Passthrough) {
            // 检查权限模式
            match permission_context.mode {
                ...
                PermissionMode::Auto => {
                    ...
                    // 检查 acceptEdits 快速路径
                    if permission_context.accept_edits_mode {
                        // 尝试转换输入类型来检查 is_read_only
                        if let Ok(typed_input) = serde_json::from_value(input.clone()) {
                            if tool.is_read_only(&typed_input) {
                                return Ok(PermissionCheckResult::allow());
                            }
                        }
                    }
                    // 调用分类器（这里简化处理，实际应该调用 AI 分类器）
                    PermissionCheckResult::ask("AI classifier needs to review this operation".to_string())
                }
                PermissionMode::Plan => {
                    // plan 模式：如果是写入操作则拒绝
                    // 尝试转换输入类型来检查 is_read_only
                    let is_write = if let Ok(typed_input) = serde_json::from_value(input.clone()) {
                        !tool.is_read_only(&typed_input)
                    } else {
                        true // 如果转换失败，假定为写入操作
                    };
                    if is_write {
    ...
    fn phase2_rule_matching<T: Tool>(
        ...
        for rule in permission_context.get_rules_by_priority(PermissionAction::Deny) {
            if rule.matches(tool_name, command.as_deref()) {
                return PermissionCheckResult::deny_with_rule(
                    rule.clone(),
                    rule.source,
                    format!("Operation denied by {} rule from {:?}", rule.target, rule.source)
                );
            }
        }
        ...
        for rule in permission_context.get_rules_by_priority(PermissionAction::Ask) {
            if rule.matches(tool_name, command.as_deref()) {
                ...
                return PermissionCheckResult::ask(
                    format!("Operation requires confirmation (rule: {})", rule.target)
                );
            }
        }
        ...
        for rule in permission_context.get_rules_by_priority(PermissionAction::Allow) {
            if rule.matches(tool_name, command.as_deref()) {
                return PermissionCheckResult::allow_with_rule(
                    rule.clone(),
                    rule.source,
                );
            }
    ...
    async fn phase3_context_check<T: Tool>(
        ...
        let typed_input = match serde_json::from_value::<T::Input>(input.clone()) {
            Ok(input) => input,
            Err(_) => return PermissionCheckResult::deny("Failed to parse input".to_string()),
        };
        let perm_result = tool.check_permissions(&typed_input, context).await;
    ...
    pub fn set(&self, value: T) -> Result<()> {
        if self.claimed.load(std::sync::atomic::Ordering::SeqCst) {
            ...
            *inner = Some(value);
            ...
            Err(anyhow::anyhow!("ResolveOnce already claimed by another caller"))
        }
    }
...
mod tests {
    ...
    fn test_classifier_check() {
        // 安全工具应该被批准
        let result = futures::executor::block_on(run_classifier_check("Read", &serde_json::Value::Null));
        assert!(result.approved);
        assert!(result.confidence > 0.5);
        
        // 不安全工具可能被拒绝
        let result = futures::executor::block_on(run_classifier_check("Bash", &serde_json::Value::Null));
        // Bash 不一定被拒绝，取决于具体实现
    }
