...
use crate::coordinator::types::{WorkerDirective, TaskNotification, TaskPhase, TaskStatus};
use crate::subagent::{SubagentType, SubagentParams, SubagentResult};
use std::sync::Arc;
...
impl Orchestrator {
    ...
    async fn spawn_task(
        ...
        let task_id = format!("agent-{}", uuid::Uuid::new_v4().to_string()[..8].to_string());
        ...
        match previous_task {
            ...
            Some(task) => {
                ...
                match (task.phase, new_purpose) {
                    ...
                    // 纠正失败，继续（有错误上下文）
                    (_, p) if p.contains("fix") || p.contains("retry") => {
                        ContinueOrSpawn::Continue
                    }
                    // 完全无关的任务，派发新的
...
impl PromptBuilder {
    ...
    pub fn with_report_requirement(mut self, requirement: &str) -> Self {
        self.prompt.push_str(&format!("报告要求：{}\n", requirement));
...
mod tests {
    ...
    async fn test_orchestrator_spawn() {
        ...
        let task_id = orchestrator.spawn_research(
            "调查 auth bug",
            "研究问题".to_string(),
        ).await;
