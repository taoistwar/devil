...
impl HookEvent {
    ...
    pub fn match_field(&self) -> Option<String> {
        match self {
            ...
            HookEvent::Notification { notification_type, .. } => Some(notification_type.clone()),
            HookEvent::Elicitation { mcp_server_name, .. } => Some(mcp_server_name.clone()),
            HookEvent::ElicitationResult { mcp_server_name, .. } => Some(mcp_server_name.clone()),
            _ => None,
        }
    }
}
