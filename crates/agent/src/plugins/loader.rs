...
use crate::plugins::types::{
    InstalledPlugin, PluginBlocklist, PluginLocation, PluginMetadata, PluginSecurityPolicy,
    ...
    fn read_metadata(&self, path: &Path) -> Result<PluginMetadata, PluginLoadError> {
        ...
        if package_json_path.exists() {
            let content = fs::read_to_string(&package_json_path).map_err(|e| {
                PluginLoadError::IoError(format!("Failed to read package.json: {}", e))
            })?;

            // 解析 package.json 提取必要字段
            let package: serde_json::Value = serde_json::from_str(&content).map_err(|e| {
                PluginLoadError::ParseError(format!("Failed to parse package.json: {}", e))
            })?;
            ...
            return Ok(PluginMetadata {
                ...
                minimum_version: package["engines"]["claude-code"]
                    .as_str()
                    .map(|s| s.to_string()),
        ...
        if plugin_json_path.exists() {
            let content = fs::read_to_string(&plugin_json_path).map_err(|e| {
                PluginLoadError::IoError(format!("Failed to read plugin.json: {}", e))
            })?;

            let metadata: PluginMetadata = serde_json::from_str(&content).map_err(|e| {
                PluginLoadError::ParseError(format!("Failed to parse plugin.json: {}", e))
            })?;
        ...
        Err(PluginLoadError::NotFound(
            "No package.json or plugin.json found".to_string(),
        ))
    }
...
impl PluginUpdater {
    ...
    pub async fn check_for_updates(
        ...
        for plugin in plugins {
            ...
            match self.fetch_latest_version(&plugin.metadata.name).await {
                ...
                Err(e) => {
                    eprintln!("Failed to check update for {}: {}", plugin.metadata.name, e);
                }
            }
        }
    ...
    pub async fn update_all(
        ...
        for update in updates {
            match self
                .update_plugin(&update.plugin_name, &update.install_path)
                .await
            {
...
mod tests {
    ...
    fn test_with_plugin_dirs() {
        ...
        let loader = PluginLoader::new().with_plugin_dirs(dirs.clone());
