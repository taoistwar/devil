...
use devil_agent::skills::{
    SkillLoader, SkillExecutor, SkillPermissionChecker,
    PermissionRule, RuleType, RuleSource,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for skill in skills {
        ...
        match checker.check(skill) {
            ...
            devil_agent::skills::PermissionCheckResult::Ask { reason, suggested_rules } => {
    ...
    if let Some(skill) = skills.first() {
        ...
        match executor.execute(skill, Some("demo argument")).await {
            Ok(result) => {
                match result {
                    devil_agent::skills::SkillExecutionResult::Inline { new_messages, context_modifier } => {
                        println!("  Mode: Inline");
                        println!("  Messages: {}", new_messages.len());
                        println!("  Allowed tools: {:?}", context_modifier.allowed_tools);
                    }
                    devil_agent::skills::SkillExecutionResult::Fork { result, agent_messages } => {
                        println!("  Mode: Fork");
    ...
    println!("Reserved budget: {} chars", budget_manager.reserved_budget());
    println!("Available budget: {} chars", budget_manager.available_budget());
    ...
    println!("Discovered {} skill directories for {:?}", discovered_dirs.len(), test_path);
