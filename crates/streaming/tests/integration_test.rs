...
use devil_streaming::{
    CacheMetrics, CacheOptimizer, CacheSafeParams, ContentReplacementState, CostTracker,
    ForkContext, ForkedAgent, ParallelPrefetcher, QueryEngine, StreamEvent, StreamingToolExecutor,
    TokenUsage, ToolState, TrackedTool, UsageDelta,
};
use futures::stream;
use std::sync::Arc;
...
async fn test_cost_tracker_budget() {
    ...
    assert_eq!(
        level,
        devil_streaming::cost_tracking::BudgetWarningLevel::Normal
    );
}
...
async fn test_cache_optimizer_health() {
    ...
    assert_eq!(
        health,
        devil_streaming::cache_optimizer::CacheHealthStatus::Excellent
    );
}
